...
public class Branch {
    ...
    //生成根和枝叶
    public synchronized static Node build(List<Node> categories,long cid,long pid){
        rootNode = new Node();
        curNode =  new Node();
        rootNode.setId(cid);
        rootNode.setParentId(pid);
        rootNode.setList(new ArrayList<Node>());
        buildTree(cid, categories, null, 0, rootNode);
        return rootNode;
    }
