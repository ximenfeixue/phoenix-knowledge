...
CREATE TABLE `tb_knowledge_base` (
  ...
  `public_date` bigint(20) DEFAULT '0' COMMENT '发布时间',
  `modify_date` bigint(20) DEFAULT '0' COMMENT '修改时间',
  `audit_status` tinyint(1) DEFAULT '1' COMMENT '审核状态(0：未通过，1：审核中，2：审核通过)',
  `status` tinyint(1) DEFAULT '1' COMMENT '状态（0为无效/删除，1为有效，2为草稿，3,：回收站）',
  ...
  `attachment_id` int(20) DEFAULT '0',
