...
CREATE TABLE `tb_knowledge_base` (
  ...
  `type` smallint(2) NOT NULL COMMENT '11种栏目类型',
  `source` varchar(255) DEFAULT NULL COMMENT '来源',
  `content_desc` varchar(255) DEFAULT NULL COMMENT '描述(知识前50个字符)',
  `title` varchar(255) DEFAULT NULL COMMENT '标题',
  `essence` tinyint(1) DEFAULT '0' COMMENT '是否加精(0-不加 1-加)',
  `visible` tinyint(2) DEFAULT '0' COMMENT '可见范围，默认0：为全平台可见，1：为自己可见，2：好友可见',
  `public_date` bigint(20) unsigned zerofill DEFAULT '0' COMMENT '发布时间',
  `modify_date` bigint(20) unsigned zerofill DEFAULT '0' COMMENT '修改时间',
  `audit_status` tinyint(1) DEFAULT '1' COMMENT '审核状态(0：未通过，1：审核中，2：审核通过)',
  `status` tinyint(1) unsigned zerofill DEFAULT '1' COMMENT '状态（0为无效/删除，1为有效，2为草稿，3,：回收站）',
  `report_status` tinyint(1) DEFAULT '1' COMMENT '举报状态(3：举报审核未通过，即无非法现象，2：举报审核通过，1:未被举报，0：已被举报)',
  `create_date` bigint(20) NOT NULL COMMENT '创建时间',
  `attachment_id` int(20) unsigned zerofill DEFAULT '0',
  `coverPic` varchar(255) DEFAULT NULL,
  `tags` varchar(255) DEFAULT NULL COMMENT '标签Id集合',
  `content` text,
  `modify_user_id` bigint(20) DEFAULT '0',
  ...
  `privated` tinyint(1) DEFAULT '0' COMMENT '是否私密 (1:是,0:否)',
...
CREATE TABLE `tb_knowledge_count` (
  ...
  `source` smallint(6) DEFAULT '0' COMMENT '知识来源(0-系统 1-用户)',
  PRIMARY KEY (`id`)
