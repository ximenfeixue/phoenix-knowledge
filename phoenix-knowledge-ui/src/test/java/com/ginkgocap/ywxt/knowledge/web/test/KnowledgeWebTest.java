...
public class KnowledgeWebTest extends BaseTestCase
{
    ...
    public void testGetKnowledgeByColumnAndSource()
    {
        LogMethod();
        try {
            String subUrl = "/allKnowledgeByColumnAndSource/1/1/2/1/20/-1"; ///allKnowledgeByColumnAndSource/{type}{columnId}/{source}/{page}/{size}/{total}
            //String urlStr =
            JsonNode result = HttpRequestFull(HttpMethod.GET, baseUrl + subUrl, null);
            checkResponseWithData(result);
    ...
    public void testAllCollectedKnowledge()
    {
        LogMethod();
        try {
            String subUrl = "/allCollected/7/1/null"; // + URLEncoder.encode("考虑", "UTF-8"); ////all/{start}/{size}/{total}/{keyword}
            JsonNode result = HttpRequestFull(HttpMethod.GET, baseUrl + subUrl, null);
            checkResponseWithData(result);
        } catch (Exception e) {
            e.printStackTrace();
