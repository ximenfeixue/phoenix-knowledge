...
public class ColumnSubscribeServiceTest extends TestBase {
	...
	public void testSearchSub() {
		System.out.println(kcsService.searchKnowledgeFromSub(1, "1", "1", 1, 1,
				10));
	}
	...
	public void testAdd() {
		...
		try {
			 ...
			 * kcs.setUserId(TEST_USER_ID); //测试所用 kcs.setUserId(79l); //测试所用
			 * kcs.setColumnId(12); //12所代表的栏目 资讯--金融 //
			 * kcs.setColumnType(1+""); // System.out.println(kcsService); //
			 * kcs=kcsService.add(kcs); System.out.println(kcs.getId());
			 * id=kcs.getId(); assertNotNull(kcs);
			 */
