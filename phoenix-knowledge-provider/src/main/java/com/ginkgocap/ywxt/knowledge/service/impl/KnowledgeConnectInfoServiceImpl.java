...
import com.ginkgocap.ywxt.people.domain.service.PeopleService;
...
//	private PersonalCustomerService personalCustomerService;
	...
	public int insertJsonAraay(String str, String tag, Long knowledgeId) {
		...
		if (json.size() > 0) {
			for (int i = 0; i < json.size(); i++) {
				...
				String type = StringUtils.equals(job.get("type") + "", "null") ? "-2"
						: job.get("type") + "";
				knowledgeconnect.setConntype(Integer.parseInt(type));
				String id = StringUtils.equals(job.get("id") + "", "null") ? "-2"
						: job.get("id") + "";
				knowledgeconnect.setConnid(Long.parseLong(id));
				if (Integer.parseInt(type) == Constants.KnowledgeConnectType.event
						.v()
						|| Integer.parseInt(type) == Constants.KnowledgeConnectType.knowledge
								.v()) {
				...
				if (Integer.parseInt(type) == Constants.KnowledgeConnectType.people
						.v()) {
					...
					if (StringUtils.equals(job.get("ptype") + "", "hy")) {
						knowledgeconnect.setUrl("/member/view/?id="
								+ job.get("id"));
					}
					Person person;
					try {
						person = personService.get(Long.valueOf(id));
						if (person != null) {

							knowledgeconnect.setPicpath(person.getPortrait());
						}
					...
					// PeopleTemp peolpletemp =
					// peopleMongoService.selectByPrimary(id);
				}
				if (Integer.parseInt(type) == Constants.KnowledgeConnectType.knowledge
						.v()) {
				...
				if (Integer.parseInt(type) == Constants.KnowledgeConnectType.organization
						.v()) {

					knowledgeconnect
							.setPicpath("/corporation/avatar/user/image/?orgid="
									+ job.get("id"));
				}
	...
	public void allAsso(String tag, String assotype, Long knowledgeId,
			Long userid) {
		...
		if (StringUtils.equals(assotype, "p")) {
			...
			List<Person> peoples = personService
					.selectByCreatorId(userid, 1, 6);
			// List<PeopleSimple> peoples =
			// peopleMongoService.getPeopleSimplelist(0, null, null, 0,
			// 6,userid, 1);
			if (peoples != null && peoples.size() > 0) {
				for (Person peopleSimple : peoples) {
					...
					knowledgeconnect.setKnowledgeid(knowledgeId);
					knowledgeconnect.setTag(tag);
					knowledgeconnect.setConnid(peopleSimple.getId());
					...
					if (list != null && list.size() > 0) {
						...
						knowledgeconnect.setConnname(StringUtils
								.trimToEmpty(peopleName.getLastname())
								+ StringUtils.trimToEmpty(peopleName
										.getFirstname()));
					}
		...
		if (StringUtils.equals(assotype, "r")) {
			conneclist = new ArrayList<ConnectionInfo>();
			Map<String, Object> map = requirementService.selectMy(userid, 1, 6,
					-1, "");
			...
			if (results != null && results.size() > 0) {
				for (Requirement requirement : results) {
					knowledgeconnect = new ConnectionInfo();
					knowledgeconnect
							.setConntype(Constants.KnowledgeConnectType.event
									.v());
					knowledgeconnect.setKnowledgeid(knowledgeId);
					...
					knowledgeconnect.setRequirementtype(requirement
							.getRequirementType() + "");
					knowledgeconnect.setUrl("/requirement/detail/"
...
//					"", "", "", "", 0, 1, 6);
		...
		if (StringUtils.equals(assotype, "k")) {
			...
			conneclist = new ArrayList<ConnectionInfo>();
			Map<String, Object> map = knowledgeHomeService
			...
			if (list != null && list.size() > 0) {
				for (int m = 0; m < list.size(); m++) {
					...
					if (mapk != null) {
						knowledgeconnect = new ConnectionInfo();
						knowledgeconnect
								.setConntype(Constants.KnowledgeConnectType.knowledge
										.v());
						knowledgeconnect.setKnowledgeid(knowledgeId);
						knowledgeconnect.setConnid(Long.parseLong(mapk
								.get("knowledge_id") + ""));
						...
						if (mapk.get("columntype") != null) {
							...
							knowledgeconnect.setColumntype(Integer
									.parseInt(mapk.get("columntype") + ""));
							knowledgeconnect.setUrl("/knowledge/reader?type="
									+ mapk.get("columntype") + "" + "&kid="
									+ mapk.get("knowledge_id") + "");
						} else {

							knowledgeconnect.setColumntype(Integer
									.parseInt(mapk.get("column_type") + ""));
							knowledgeconnect.setUrl("/knowledge/reader?type="
									+ mapk.get("column_type") + "" + "&kid="
									+ mapk.get("knowledge_id") + "");
						}
						conneclist.add(knowledgeconnect);
					}
