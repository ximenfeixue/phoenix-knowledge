...
@Component("knowledgeIndustryAdminDAO")
public class KnowledgeIndustryAdminDAOImpl implements KnowledgeIndustryAdminDAO {
	...
	@Override
	public List<KnowledgeIndustry> findAll() {
		return mongoTemplate.findAll(KnowledgeIndustry.class, "KnowledgeIndustry");
	}

	@Override
	public Map<String,Object> selectKnowledgeIndustryList(Integer page,
		...
		long count = mongoTemplate.count(query, "KnowledgeIndustry");
		PageUtil p = new PageUtil((int) count, page, size);
		query.limit(size);
		query.skip(p.getPageStartRow() - 1);
		List<KnowledgeIndustry> list = mongoTemplate.find(query, KnowledgeIndustry.class, "KnowledgeIndustry");
	...
	@Override
	public long selectKnowledgeIndustryListCount() {
		Criteria criteria = Criteria.where("cid").is(1);
		Query query = new Query(criteria);
		long count = mongoTemplate.count(query, KnowledgeIndustry.class);
		return count;
	}

	@Override
	public KnowledgeIndustry selectKnowledgeIndustryById(long id) {
		KnowledgeIndustry news = mongoTemplate.findById(id, KnowledgeIndustry.class, "KnowledgeIndustry");
		return news;
	}
	...
	public void deleteKnowledgeIndustryById(long id) {
		KnowledgeIndustry news = mongoTemplate.findById(id, KnowledgeIndustry.class, "KnowledgeIndustry");
		mongoTemplate.remove(news, "KnowledgeIndustry");
	}
	...
	public void checkStatusById(long id, int status) {
		...
		mongoTemplate.save(news, "KnowledgeIndustry");
	}
}
